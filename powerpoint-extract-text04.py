...
# CSVファイルの読み込み
data = pd.read_csv('speaker/speaker-id.csv')
...
if uploaded_file:
    # characterの選択
    character = st.selectbox('キャラクターを選択:', data['character'].unique())

    # 選択されたcharacterに基づいてstyleを選択
    filtered_styles = data[data['character'] == character]['style'].unique()
    style = st.selectbox('スタイルを選択:', filtered_styles)

    # 選択されたcharacterとstyleに基づいてidを選択
    filtered_ids = data[(data['character'] == character) & (data['style'] == style)]['id'].unique()
    speaker = st.selectbox("Choose a speaker voice", filtered_ids)  # Speaker IDの選択肢
